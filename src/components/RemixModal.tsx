...
import { Sparkles, Loader2, Music, Download, CheckCircle2 } from "lucide-react";
...
export default function RemixModal({
    ...
    return (
        <Dialog open={isOpen} onOpenChange={handleClose}>
            <DialogContent className={`${step === "result" ? "sm:max-w-2xl" : "sm:max-w-[500px]"} border-2 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]`}>
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Transform {originalLyrics.split('\n')[0].substring(0, 30)}... into a new masterpiece!
                    </DialogDescription>
                </DialogHeader>
                ...
                {step === "loading" && (
                    <div className="flex flex-col items-center justify-center py-12 space-y-4">
                        ...
                        <div className="text-center space-y-1">
                            <h3 className="text-lg font-bold">Generative Magic in Progress...</h3>
                            ...
                            <div className="flex-1 rounded-2xl border-2 border-border/60 bg-muted/30 p-6 font-mono text-sm shadow-inner group overflow-hidden">
                                <div className="h-full overflow-y-auto pr-2 custom-scrollbar space-y-2.5">
                                    {result.parodyLrcLines.map((line, i) => (
                                        <div key={i} className="flex gap-4 items-start group/line py-1 border-b border-border/5 last:border-0">
                                            <span className="text-[11px] font-bold text-primary/40 mt-1 tabular-nums w-14 shrink-0 bg-primary/5 rounded px-2 py-0.5 text-center">
                                                {Math.floor(line.timestamp / 60)}:{(line.timestamp % 60).toFixed(1).padStart(4, "0")}
                                            </span>
