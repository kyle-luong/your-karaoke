...
// Gradient fallbacks for songs without thumbnails
const GRADIENTS = [
...
// Category slug → display name + genre filter keywords
const CATEGORY_MAP: Record<string, { name: string; description: string; genres: string[] }> = {
  rap: {
    name: "Rap & Hip-Hop",
...
// Songs to pin at the top of specific categories (shown first, then remaining genre songs follow)
const PINNED_SONGS: Record<string, string[]> = {
  rap: ["I Wonder", "Humble"],
};
...
export default async function CategoryPage({ params }: PageProps) {
  ...
  const categoryDescription = category?.description ?? "Browse songs in this category.";

  let songs: Song[] = [];

  if (category) {
    // Build an OR filter for multiple genre keywords (e.g. "Hip-Hop" or "Rap")
    const orFilter = category.genres
      .map((g) => `genre.ilike.%${g}%`)
      .join(",");
  ...
  // Pin specific songs at the top if configured
  const pinned = PINNED_SONGS[categoryId];
  if (pinned) {
    const { data: pinnedData } = await supabase
      .from("songs")
      .select("*")
      .in("title", pinned);
    const pinnedSongs = (pinnedData as Song[]) ?? [];
    pinnedSongs.sort((a, b) => pinned.indexOf(a.title) - pinned.indexOf(b.title));

    const pinnedIds = new Set(pinnedSongs.map((s) => s.id));
    const rest = songs.filter((s) => !pinnedIds.has(s.id));
    songs = [...pinnedSongs, ...rest];
  }
