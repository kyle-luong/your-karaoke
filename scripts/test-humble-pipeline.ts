...
async function main() {
  ...
  lines.forEach((l) =>
    console.log(`   [${(l.timeMs / 1000).toFixed(2)}s] ${l.line}`),
  );
...
Respond ONLY with valid JSON matching this exact schema:
{
  ...
  for (const l of parodyResult.parodyLrcLines) {
    console.log(`   [${l.timeMs}ms] ${l.line}`);
  }
  ...
  for (const line of parodyResult.parodyLrcLines) {
    process.stdout.write(`   Synthesizing: "${line.line}"...`);
  ...
  try {
    ...
    const instrumentalOut = join(OUTPUT_DIR, "humble-instrumental-20s.mp3");
