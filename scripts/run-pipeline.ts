...
import { convertVoice, pickVoiceForGenre } from "../src/lib/services/voice-conversion";
...
const OUTPUT_DIR = join(process.cwd(), `public/generated/${SONG_SLUG}-parody-final`);
...
Original LRC lines (timestamp in seconds + text):
${lrcBlock}

CRITICAL RULES:
1. Rewrite EVERY line individually.
2. Keep the EXACT same number of lines.
3. Keep the EXACT same timestamps.
...
Respond ONLY with valid JSON:
{
  ...
  "generatedAt": "string"
    ...
    const segments: Array<{ timestamp: number; audioBuffer: Buffer }> = [];

    for (let i = 0; i < parodyResult.parodyLrcLines.length; i++) {
        const line = parodyResult.parodyLrcLines[i];
        process.stdout.write(`   [Line ${i + 1}/${parodyResult.parodyLrcLines.length}] Synthesizing...`);
        const audioBuffer = await synthesizeLine(line.text, effectiveVoiceId, elevenLabsKey);
        console.log(` ✓`);
        segments.push({ timestamp: line.timestamp, audioBuffer });
    }
    ...
    try {
        ...
        execFileSync("ffmpeg", ["-y", "-i", INSTRUMENTAL_PATH, "-c:a", "libmp3lame", "-q:a", "2", instrumentalPath]);
        ...
        const buildMix = (includeInstrumental: boolean, outputName: string) => {
            ...
            filters.push(`${vocalInputs}amix=inputs=${segments.length}:duration=longest:dropout_transition=0:normalize=0[vocals]`);

            if (includeInstrumental) {
                filters.push(`[0:a]volume=0.25[inst];[inst][vocals]amix=inputs=2:duration=first:dropout_transition=0:normalize=0[out]`);
        ...
        execFileSync("ffmpeg", ["-y", "-i", finalWav, "-c:a", "libmp3lame", "-q:a", "2", finalMp3]);
        execFileSync("ffmpeg", ["-y", "-i", vocalWav, "-c:a", "libmp3lame", "-q:a", "2", vocalMp3]);
        ...
        if (USE_PRO_AUTOTUNE) {
            ...
            try {
                ...
                execFileSync("python3", [
                    ...
                    "--input", vocalMp3,
                    "--output", autotunedVocals,
                    "--strength", String(AUTOTUNE_STRENGTH)
                ]);
                ...
                execFileSync("ffmpeg", [
                    "-y", "-i", autotunedVocals, "-i", instrumentalPath,
                    "-filter_complex", filters, "-map", "[out]",
                    "-c:a", "libmp3lame", "-q:a", "2", autotunedMixed
                ]);
        ...
        if (USE_VOICE_CONVERSION) {
            ...
            try {
                ...
                execFileSync("ffmpeg", [
                    "-y", "-i", convertedVocalsPath, "-i", instrumentalPath,
                    "-filter_complex", mixFilters, "-map", "[out]",
                    "-c:a", "libmp3lame", "-q:a", "2", convertedMixPath,
                ]);
                finalMixedToSave = convertedMixPath;

                console.log("   ✅ Voice conversion complete.");
            } catch (e) {
                console.error("   ❌ Voice conversion failed, using previous vocals.", e);
            }
        }

        mkdirSync(OUTPUT_DIR, { recursive: true });
        writeFileSync(join(OUTPUT_DIR, `${SONG_SLUG}-parody-mixed.mp3`), readFileSync(finalMixedToSave));
        writeFileSync(join(OUTPUT_DIR, `${SONG_SLUG}-vocals-only.mp3`), readFileSync(finalVocalsToSave));

        const lrcText = parodyResult.parodyLrcLines.map((l: any) => {
            const min = Math.floor(l.timestamp / 60);
            const sec = (l.timestamp % 60).toFixed(2).padStart(5, "0");
            return `[${String(min).padStart(2, "0")}:${sec}]${l.text}`;
        }).join("\n");
        writeFileSync(join(OUTPUT_DIR, `${SONG_SLUG}-lyrics.lrc`), lrcText);
        writeFileSync(join(OUTPUT_DIR, "metadata.json"), JSON.stringify(parodyResult, null, 2));
